"""Unified resilience patterns for system stability."""
import time
import logging
...
class CircuitBreaker:
    """
    Unified circuit breaker pattern supporting both API resilience and trading halt.
    ...
    For API resilience: Use can_execute() / record_success() / record_failure()
    For trading halt: Use check_can_trade() / trigger_circuit_breaker() / reset()
    """
    ...
    def __init__(
        ...
        Args:
            ...
            mode: "api" for API resilience or "trading" for trading halt
        """
        ...
        # Trading thresholds
        self.DRAWDOWN_WARNING_PCT = 0.10
        self.DRAWDOWN_HALT_PCT = 0.15
        self.DAILY_LOSS_LIMIT_PCT = 0.03
        self.CONSECUTIVE_LOSS_LIMIT = 5
        self.KILL_SWITCH_FILE = "/tmp/trading_stop"

    def can_execute(self) -> bool:
        """
        Check if operation should be allowed (API mode).
    ...
    def check_can_trade(
        ...
        """
        Check if trading is allowed (trading mode).
